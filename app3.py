...
def clean_text(text, keep_emojis, keep_stopwords, keep_punctuation, keep_digits, language):
    if not keep_emojis:
...
keep_digits = st.sidebar.radio("Keep the digits?", ('Yes', 'No')) == 'Yes'
...
if st.session_state.clean_text_button_clicked and text:
    ...
    digits_in_text = detect_digits(text)
    ...
    if keep_digits and digits_in_text:
        st.warning(f"Digits detected: {', '.join(digits_in_text)}")

    cleaned_text = clean_text(text, keep_emojis, keep_stopwords, keep_punctuation, keep_digits, language)
